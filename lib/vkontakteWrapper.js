class VkontakteWrapper {
	...
	getUserId(){
		this.sdk.send("VKWebAppGetUserInfo", {})
				  .then((data) => {
				  	console.log(data.id);
      				return data.id;
    });
	...
	freindsInvite(){
		this.sdk.send('VKWebAppShowInviteBox')
		  .then( (data) => {
unityInstance.SendMessage('vkobj', 'FreindsInvite', data.success);
  });
	}

	addToProfile(){
		this.sdk.send('VKWebAppAddToProfile', {
  ttl: 0
  })
  .then((data) => { 
    if (data.visibility) {
